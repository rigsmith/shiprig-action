...
Every row is done. Rows 1–6 and 8: the rigsmith side shipped in shiprig
1.20.0, the action side in #7. Row 7: shiprig 1.21.0 and shiprig-action 0.6.0.

| #   | Upstream (npm-only)                                                                           | shiprig-action                                                                                                                                                                                                                                                                                                           | Status                            |
| --- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------- |
| 1   | `@changesets/cli` from `node_modules` (`execChangesetsCli`, `validateChangesetsCliVersion`)   | `shiprig` on `PATH`, or at `$SHIPRIG_BIN`. The job installs it; the action doesn't. Checked before anything runs: `shiprig --version` prints the bare version when piped (1.21.0 on), and anything below the minimum, 1.21.0 (an older shiprig prints its banner), fails naming it; a source build passes with a warning | Done                              |
...
| 7   | `changeset publish-plan`, `pack`, `publish --from-pack-dir` (the split sub-actions)           | `shiprig publish-plan`, `shiprig pack`, `shiprig publish --from-pack-dir` (shiprig 1.21.0, the same file format). `select-mode` plans with shiprig, `pack` packs with it, and `publish` publishes the packed files; cargo is refused at pack, since it publishes from source only                                        | Done                              |
...
- **`pr-status` runs on shiprig**, ahead of the rest of phase 4, since it needs
  ...
  `status --since`: only the PR's changesets, and since shiprig 1.21.0 only its
  commits too). In a changesets-only repository it only runs when the PR adds
  or edits a changeset, since that command is also the CI gate and fails on
  changed packages with none; with commits as a source, a PR can release from
  its commits alone. The changed packages for the "add a changeset" link come
  from `shiprig packages list`, so any ecosystem works. The comment adds a
  changelog preview from `shiprig version --changelog --since <base>`.
